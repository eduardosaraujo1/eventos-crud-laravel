...
@section('content')
    <section class="events-hero">
        <div inert class="events-hero-bg"></div>
        <div class="container">
            <div class="table-header">
                <div class="w-100">
                    ...
                    <div class="title-and-action">
                        ...
                        <a href="{{ route('eventos_create') }}" class="btn btn-primary">Novo Evento</a>
                    </div>
                </div>
            </div>
            @if ($eventos->isEmpty())
                <h3 style="font-weight: 500;">Não há eventos agendados no momento.</h3>
            @else
                        ...
                        <tbody>
                            @foreach ($eventos as $e)
                                <tr>
                                    <td>{{ $e->id }}</td>
                                    <td>{{ $e->nome }}</td>
                                    <td><span class="badge badge-purple">R$ {{ number_format($e->preco, 2, ',', '.') }}</span></td>
                                    <td>{{ $e->quantidade }}</td>
                                    <td>{{ $e->created_at->format("Y-m-d") }}</td>
                                    <td>
                                        <div class="evento-acoes">
                                            <div>
                                                <a href="{{ route('eventos_edit', $e->id) }}" class="btn btn-primary">Editar</a>
                                            </div>
                                            <form action="{{ route('eventos_destroy', $e->id) }}" method="post">
                                                @csrf
                                                @method('delete')
                                                <button class="btn btn-danger"
                                                    onclick="return confirm('Tem certeza que deseja excluir \'{{ $e->nome }}\'?')">Excluir</button>
                                            </form>
                                        </div>
                                    </td>
                                </tr>
